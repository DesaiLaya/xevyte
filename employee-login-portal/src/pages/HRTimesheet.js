...
const HRTimesheetsTable = ({ hrId, passedEmployeeId, searchTerm }) => {
    ...
    useEffect(() => {
        const fetchAllTimesheets = async () => {
            ...
            try {
                ...
                const response = await axios.get(`http://localhost:8082/daily-entry/hr/${hrId}`);
    ...
    // Filter and sort entries based on all filters, including month/year
    ...
    const handleDownload = () => {
        ...
        XLSX.writeFile(workbook, `hr_timesheet_data_${new Date().toLocaleDateString()}.xlsx`);
