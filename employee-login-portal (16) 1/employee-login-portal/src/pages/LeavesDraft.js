...
function LeavesDrafts() {
  ...
  useEffect(() => {
    async function loadDrafts() {
      ...
      setDraftsError("");
      try {
        const res = await fetch(`http://localhost:8082/leaves/drafts/${employeeId}`);
        if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
        const data = await res.json();

        // Sort drafts by ID desc (most recent first)
        const sortedDrafts = (Array.isArray(data) ? data : []).sort((a, b) => (b.id ?? 0) - (a.id ?? 0));
  ...
  // ✅ Delete draft from backend and update list
  const handleDeleteDraft = async (draftId) => {
    try {
      const res = await fetch(`http://localhost:8082/leaves/drafts/${draftId}`, {
        method: "DELETE",
      });
      if (res.ok) {
                      ...
                      {filteredDrafts.map((draft) => (
                        <tr key={draft.id} style={{ borderBottom: '1px solid #eee' }}>
                          ...
                          <td style={{ padding: '12px', border: '1px solid #ddd' }}>
                            {draft.fileName ? (
                              <a
                                href={`http://localhost:8082/leaves/drafts/download/${draft.id}`}
