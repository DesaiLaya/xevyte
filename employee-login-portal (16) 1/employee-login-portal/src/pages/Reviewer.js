...
function ReviewerGoals() {
  ...
  useEffect(() => {
    if (!role) {
      setErrorMessage('User role not found. Please log in.');
      setIsAuthorized(false);
      return;
    }

    const roleLower = role.toLowerCase();
    if (roleLower !== 'reviewer') {
      setErrorMessage('Access denied. You are not a reviewer.');
      setIsAuthorized(false);
      return;
    }
    ...
    const url = `http://localhost:8082/api/goals/reviewer/${employeeId}/employees`;
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="main-content">
        ...
        <div className="emp-container">
          <h2 className="emp-title">Employees under Reviewer {employeeId}</h2>
          {errorMessage && <p className="emp-error">{errorMessage}</p>}
          {!errorMessage && employees.length === 0 && <p className="emp-empty">No employees found.</p>}
          {employees.length > 0 && (
