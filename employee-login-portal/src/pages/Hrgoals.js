...
function HrGoals() {
  ...
  useEffect(() => {
    if (!role || role.toLowerCase() !== 'hr') {
      setErrorMessage('Access denied. You are not an HR user.');
      setIsHR(false);
      return;
    }
    ...
    fetch(`http://localhost:8082/api/goals/hr/${employeeId}/employees`)
      .then((response) => {
          ...
          <>
            ...
            <Link to="/hom" className="hom" style={{ textDecoration: 'none' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                Favourites <img src={require("../assets/star4.png")} alt="office" style={{ width: '22px', height: '22px' }} />
              </span>
            </Link>
