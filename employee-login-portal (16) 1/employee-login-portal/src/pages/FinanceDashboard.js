...
function FinanceDashboard() {
  ...
  useEffect(() => {
    ...
    if (storedId && storedRole === "Finance") {
  ...
  const fetchClaims = (financeId) => {
    axios
      ...
      .then((response) => {
        ...
        setOriginalClaims(sortedClaims); // Save the sorted original data
      })
      .catch((error) => console.error("Error fetching assigned finance claims:", error));
  };
            ...
            <div className="header-right" style={{ display: 'flex', alignItems: 'center', gap: '10px', position: 'relative' }}>
              ...
              <div className="profile-wrapper" style={{ position: 'relative' }}>
                ...
                {profileOpen && (
                  ...
                  >
                    <button onClick={handleEditProfile} style={{ padding: '10px', width: '100%', border: 'none', textAlign: 'left', cursor: 'pointer', borderBottom: '1px solid #eee' }}>
                      Edit Profile
                    </button>
                    <button onClick={handleLogout} style={{ padding: '10px', width: '100%', border: 'none', textAlign: 'left', cursor: 'pointer' }}>
                      Logout
                    </button>
                  </div>
                )}
              ...
              <tbody>
                {claims.map(claim => (
                  <tr key={claim.id}>
                    ...
                    <td>
                      {claim.receiptName ? (
                        <button onClick={() => handleViewReceipt(claim.id, claim.receiptName)} style={{ color: "blue", textDecoration: "underline", backgroundColor: "transparent", cursor: "pointer" }}>
                          {claim.receiptName}
                        </button>
                      ) : "No Receipt"}
                    </td>
